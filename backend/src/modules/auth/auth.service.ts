...
export class AuthService {
    ...
    async logoutUserService(data: logoutUserDTO) {
        const { refreshToken } = data;

        const hashedRefreshToken = hashRefreshToken(refreshToken);

        const exisitingRefreshToken = await this.userRepo.findRefreshToken(hashedRefreshToken);

        if (!exisitingRefreshToken) {
            throw new AppError("Invalid refresh token", 401);
        }

        await this.userRepo.deleteRefreshTokenById(exisitingRefreshToken.id);

        return true;
    }

    async logoutAllDevicesService(userId: string) {
        await this.userRepo.deleteAllRefreshTokenByUserId(userId);

        return true;
    }

    async refreshTokenService(data: refreshTokenDTO) {
        const { refreshToken } = data;
        ...
        const existingRefreshToken = await this.userRepo.findRefreshToken(hashedOldRefreshToken);
